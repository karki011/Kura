...
For free on-device transcription **and** speaker separation, choose **On-device · speaker labels** in Settings → Audio. Parakeet streaming ASR and LS-EEND diarization run as on-device CoreML models via [FluidAudio](https://github.com/FluidInference/FluidAudio); the models download once on first use and work offline afterwards. [On-device speech notes](Sources/Kura/Resources/LOCAL_SPEECH_SETUP.md) have the details. No Python or manual model setup is required. Deepgram has been removed.

On-device text streams live and each utterance commits with its speaker label shortly after the speaker pauses. Labels (up to 10 speakers with the default LS-EEND separation; a max-4 Sortformer option in Settings trades capacity for steadier identities) are stable within a session but tentative; returning speakers after long pauses may receive a new label, and names still need confirmation. Audio never leaves the Mac. Optional microphone inclusion remains a separate Apple recognition path.
